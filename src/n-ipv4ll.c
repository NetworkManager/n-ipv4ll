...
static int n_ipv4ll_push_event(NIpv4ll *ll, unsigned int event, struct in_addr *address, uint16_t *operation, struct ether_addr *sender, struct in_addr *target) {
        NIpv4llEventNode *node;
        int r;
...
static int n_ipv4ll_handle_acd_event(NIpv4ll *ll, NAcdEvent *event) {
        ...
        case N_ACD_EVENT_READY:
                r = n_ipv4ll_push_event(ll, N_IPV4LL_EVENT_READY, &ll->config.ip, NULL, NULL, NULL);
                if (r < 0)
                        return r;
        ...
        case N_ACD_EVENT_DEFENDED:
                r = n_ipv4ll_push_event(ll, N_IPV4LL_EVENT_DEFENDED, NULL, &event->defended.operation, &event->defended.sender, &event->defended.target);
                if (r < 0)
                        return r;
        ...
        case N_ACD_EVENT_CONFLICT:
                r = n_ipv4ll_push_event(ll, N_IPV4LL_EVENT_CONFLICT, NULL, &event->conflict.operation, &event->conflict.sender, &event->conflict.target);
                if (r < 0)
                        return r;
        ...
        case N_ACD_EVENT_DOWN:
                r = n_ipv4ll_push_event(ll, N_IPV4LL_EVENT_DOWN, NULL, NULL, NULL, NULL);
                if (r < 0)
                        return r;
